...
UbxGps::UbxGps(SoftwareSerial &serial) : serial(serial)
{
...
boolean UbxGps::ready()
{
    ...
    while (this->available())
    {
        ...
        else
        {
            ...
            // Carriage is at the first checksum byte, we can calculate our checksum, but not compare, because this byte
            // is not read
            if (p == (this->size + 2))
            {
